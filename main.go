...
func initFlags() {
	...
	rootCmd.PersistentFlags().StringVarP(&config.ModelID, flagModel, string(flagModel[0]), "", "The specific foundation model to use (default is claude-sonnet-5)")
	...
	rootCmd.PersistentFlags().BoolVarP(&config.Think, flagThink, "k", false, "Enable thinking (extended for 3.7-4.5, adaptive for Opus 4.6/4.7/4.8, Opus 5, Sonnet 4.6, Sonnet 5, and Fable 5)")
	rootCmd.PersistentFlags().IntVarP(&config.BudgetTokens, flagBudget, string(flagBudget[0]), 0, fmt.Sprintf("Thinking token budget for Claude 3.7-4.5; ignored for Opus 4.6/4.7/4.8, Opus 5, Sonnet 5, and Fable 5, use --effort instead (default=%d)", defaultThinkingTokens))
	...
	rootCmd.PersistentFlags().StringVarP(&config.Effort, flagEffort, "E", "", "Effort level (max, xhigh, high, medium, low). 'xhigh' is Opus 4.7/4.8, Opus 5, Sonnet 5, and Fable 5; 'max' is Opus 4.6/4.7/4.8, Opus 5, Sonnet 5, and Fable 5.")
