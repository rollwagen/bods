...
func initFlags() {
	...
	rootCmd.PersistentFlags().StringVarP(&config.ModelID, "model", "m", ClaudeV2.String(), "the specific foundation model to use")
	// format flag to Ask for the response to be formatted as markdown unless otherwise set.
...
func main() {
	...
	if err != nil {
		handleError(bodsError{err, "Could not load your configuration file."})
		os.Exit(1)
	}
